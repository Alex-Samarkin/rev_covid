...
println("Доступные волны: ", sort(unique(df_all.wave)))

for WAVE_NUM in sort(unique(df_all.wave))

    local df_wave = sort(filter(:wave => ==(WAVE_NUM), df_all), :date)
    ...
    best_scan = (err=Inf, N_eff=N_eff_fs)
    for N_eff_try in scan_Ns
        ...
        if err < best_scan.err
            best_scan = (err=err, N_eff=N_eff_try)
        end
    end
    ...
    # В) Компартменты
    S_v = sol_fit[1, 1:n_plt]
    E_v = sol_fit[2, 1:n_plt]
    I_v = sol_fit[3, 1:n_plt]
    R_v = sol_fit[4, 1:n_plt]
    D_v = sol_fit[5, 1:n_plt]
    ...
    display(panel)
    savefig(panel, "./figures/covid/seird_wave$(WAVE_NUM)6_fit.png")
    savefig(panel, "./figures/covid/seird_wave$(WAVE_NUM)6_fit.svg")
    println("\nГрафики → figures/covid/seird_wave$(WAVE_NUM)6_fit.(png|svg)")
    ...
    @save "./data_out/seird_wave$(WAVE_NUM)6_results.jld2" df_traj df_par
    CSV.write("./data_out/seird_wave$(WAVE_NUM)6_trajectories.csv", df_traj)
    CSV.write("./data_out/seird_wave$(WAVE_NUM)6_params.csv", df_par)
